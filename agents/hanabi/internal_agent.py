...
class InternalAgent(IGGIAgent):
    def _find_first_playable_hint_mask(self, partner_hand, next_playable_rank, avail_mask):
        ...
        masked_indices = jnp.where(partner_playable, slot_indices, self.hand_size)
        first_slot = jnp.argmin(masked_indices)
        has_playable = jnp.any(partner_playable)

        chosen_color = partner_colors[first_slot]
        chosen_rank = partner_ranks[first_slot]
        fire = has_playable.astype(jnp.float32)

        color_mask = jnp.zeros(self.num_colors)
        color_mask = color_mask.at[chosen_color].set(fire)
        rank_mask = jnp.zeros(self.num_ranks)
        rank_mask = rank_mask.at[chosen_rank].set(fire)
    ...
    def _get_action(
        ...
        hint_playable_mask = self._find_first_playable_hint_mask(
